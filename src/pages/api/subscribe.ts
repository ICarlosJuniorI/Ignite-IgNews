...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === 'POST') {
    ...
    const stripeCheckoutSession = await stripe.checkout.sessions.create({
      customer: customerId,
      payment_method_types: ['card'], // Método de pagamento aceito
      billing_address_collection: 'required', // Necessário preencher o endereço
      line_items: [ // Item e quantidade
        { price: 'price_1K2gSlCCjSt7w1sNegL8Q42a', quantity: 1 }
      ],
      mode: 'subscription', // Pagamento recorrente
      allow_promotion_codes: true, // Permite cupom
